...
- تعرض الصفحة اسم قاعدة البيانات، المعرّف التشغيلي، المزود، وبيانات الخادم الآمنة (host/port أو
  المفتاح التشغيلي)، ثم حالة المورد ومساحة العمل وآخر فحص اتصال وصحته والـschema والنسخ المرتبطة.
  لا تعرض connection string أو القيمة المشفرة.
- زر `تسجيل قاعدة بيانات` يرسل الاسم والصفة وسلسلة الاتصال إلى الخادم عبر TLS؛ الخادم يختبرها
  ويشفّرها فورًا ويحفظ نتيجة الفحص الآمنة. إذا فشل الاتصال يبقى المورد ظاهرًا كـ`Faulted` مع كود
  وسبب قابل للفهم بدل اختفاء الخطأ.
- لكل صف زر `فحص الاتصال` يختبر القيمة المحمية داخل الخادم، و`إصلاح` يستبدلها فقط بعد إدخال
  قيمة جديدة واختبارها. يظهر `حذف نهائي` فقط للمورد غير المرتبط وغير المستخدم؛ يحمي الخادم الحذف
  إذا وجد mapping أو backup أو تجهيز/استعادة نشطة، ويعرض سبب الحماية.
- `Refresh` يعيد قراءة الحالة، مع ترقيم خادمي وحالات Loading/Empty/Error. الترحيلات والنسخ
  والتخصيص تتم عبر APIs محمية، ولا يخرج secret إلى المتصفح أو السجل.
...
عند إرسال طلب معلومات إضافية يمكن اختيار `PaymentProof` كإجراء رفع خاص؛ لا يدخل هذا الاسم في
JSON بيانات المساحة، بل يرفع المالك الملف من شاشة المتابعة، ثم يظهر الإصدار المحفوظ هنا للمعاينة
والتحقق قبل اعتماد الدفع.
