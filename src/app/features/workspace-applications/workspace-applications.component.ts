...
export class WorkspaceApplicationsComponent implements OnInit {
  ...
  readonly paymentOptions = [{ label: 'الدفع قيد المراجعة', value: PaymentRequestStatus.Pending }, { label: 'الدفع معتمد', value: PaymentRequestStatus.Approved }, { label: 'الدفع مرفوض', value: PaymentRequestStatus.Rejected }];
  readonly provisioningOptions = [{ label: 'بانتظار السعة', value: ProvisioningJobStatus.AwaitingDatabaseCapacity }, { label: 'جاري التجهيز', value: ProvisioningJobStatus.Provisioning }, { label: 'مكتمل', value: ProvisioningJobStatus.Completed }, { label: 'فشل', value: ProvisioningJobStatus.Failed }];
  ...
  load(): void { this.loading.set(true); this.service.list({ status: this.statusFilter ?? undefined, type: this.typeFilter ?? undefined, paymentStatus: this.paymentFilter ?? undefined, provisioningStatus: this.provisioningFilter ?? undefined }, this.page, this.pageSize).subscribe({ next: data => { this.rows.set(data.items); this.totalCount = data.totalCount; this.loading.set(false); }, error: err => { this.notify.error(errMsg(err)); this.loading.set(false); } }); }
  ...
  approvePayment(application: PlatformWorkspaceApplication): void { if (!application.paymentRequestId) return; void this.notify.confirm({ header: 'اعتماد الدفع', message: 'سيصبح الطلب مؤهلاً لاعتماد المساحة والتجهيز. متابعة؟', acceptLabel: 'اعتماد الدفع', icon: 'pi pi-check-circle' }).then(ok => { if (!ok) return; this.busyId.set(application.id); this.paymentService.approve(application.paymentRequestId!).subscribe({ next: () => { this.busyId.set(null); this.notify.success('تم اعتماد الدفع.'); this.load(); }, error: err => { this.busyId.set(null); this.notify.error(errMsg(err)); } }); }); }
  openPaymentReject(application: PlatformWorkspaceApplication): void { this.paymentRejectTarget.set(application); this.paymentRejectReason = ''; this.showPaymentReject = true; }
  confirmPaymentReject(): void { const application = this.paymentRejectTarget(); if (!application?.paymentRequestId || !this.paymentRejectReason.trim()) { this.notify.error('أدخل سبب رفض الدفع.'); return; } this.busyId.set(application.id); this.paymentService.reject(application.paymentRequestId, { rejectReason: this.paymentRejectReason.trim() }).subscribe({ next: () => { this.busyId.set(null); this.showPaymentReject = false; this.notify.success('تم رفض الدفع.'); this.load(); }, error: err => { this.busyId.set(null); this.notify.error(errMsg(err)); } }); }
  ...
  private async confirmThen(application: PlatformWorkspaceApplication, header: string, message: string, request: () => Observable<PlatformWorkspaceApplication>, success: string): Promise<void> { if (await this.notify.confirm({ header, message, acceptLabel: 'تأكيد', icon: 'pi pi-check-circle' })) this.run(application, request, success); }
