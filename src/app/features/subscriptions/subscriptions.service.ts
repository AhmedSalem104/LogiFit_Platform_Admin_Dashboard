...
import { PagedResult, PlatformSubscriptionDto, TenantSubscriptionStatus } from '../../core/models/platform.models';
...
export class SubscriptionsService {
  ...
  list(status?: TenantSubscriptionStatus, page = 1, pageSize = 20): Observable<PagedResult<PlatformSubscriptionDto>> {
    let params = new HttpParams().set('page', page).set('pageSize', pageSize);
    if (status != null) params = params.set('status', status);
    return this.http.get<PagedResult<PlatformSubscriptionDto>>(this.base, { params });
  }
