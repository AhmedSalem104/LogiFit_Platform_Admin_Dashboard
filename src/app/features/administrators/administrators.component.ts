...
export class AdministratorsComponent implements OnInit {
  ...
  create(): void { if (!this.canCreate() || this.creating()) return; this.creating.set(true); this.http.post(`${environment.apiUrl}/administrators`, { fullName: this.fullName.trim(), email: this.email.trim(), password: this.password }).subscribe({ next: () => { this.creating.set(false); this.showCreate = false; this.notify.success('تم إنشاء المشرف'); this.load(); }, error: (error) => { this.creating.set(false); this.notify.error(errMsg(error)); } }); }
  async toggle(row: any): Promise<void> { if (this.busyId()) return; const action = row.isActive ? 'تعطيل' : 'تفعيل'; this.busyId.set(row.id); const ok = await this.notify.confirm({ header: `${action} المشرف`, message: `هل تريد ${action} حساب ${row.email}؟`, acceptLabel: action, danger: row.isActive }); if (!ok) { this.busyId.set(null); return; } this.http.patch(`${environment.apiUrl}/administrators/${row.id}/status`, !row.isActive).subscribe({ next: () => { this.busyId.set(null); this.notify.success(`تم ${action} الحساب`); this.load(); }, error: (error) => { this.busyId.set(null); this.notify.error(errMsg(error)); } }); }
}
